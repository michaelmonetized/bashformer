# Vex Mobile Ports - Structure Overview
...
│   └── vex.c           # Original game code
├── ios/                 # iOS Swift version
│   ├── VexSDL/         # Swift source files
│   │   ├── GameScene.swift
│   │   ├── GameState.swift
...
- **VexGameView.java**: Custom SurfaceView for game rendering and touch handling
- **VexGameJNI.java**: JNI interface definitions
- **vex_game_jni.c**: JNI wrapper implementation bridging Java and C
...
cd ..
git submodule add ./vex_sdl vex_sdl
```

Or if you want it as a separate repository:

```bash
# In vex_sdl directory
git init
...
# In parent directory
git submodule add <your-repo-url> vex_sdl
```
