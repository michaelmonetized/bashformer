// Vex - A retro platformer game in an SDL2 window.
// Controls (Vim-style):
// - h / Left: move left
...
// - Esc: quit
//
// Dependencies: SDL2 + SDL2_image + SDL2_mixer (for PNG sprites and audio)
// Install SDL2_mixer: 
...
//   gcc vex.c -o vex $(sdl2-config --cflags --libs) -lSDL2_image -lSDL2_mixer -lm
// or (with pkg-config):
//   gcc vex.c -o vex `pkg-config --cflags --libs sdl2 SDL2_image SDL2_mixer` -lm
...
// Final asset paths: ./sprites/vex.png, ./sprites/hero.png, etc. (relative to working directory)
//
...
typedef struct {
    RectF rect;
    int active;
    float vx, vy;  // Velocity for exploding coins from baddie death
    float lifetime; // Time remaining before coin disappears (for exploded coins)
} Coin;
...
// Audio system
typedef struct {
    Mix_Music *bgMusic;          // Background MIDI music
    Mix_Chunk *sfxKillBaddie;    // Kill baddie sound
    Mix_Chunk *sfxBreakBarrel;   // Break barrel sound
    Mix_Chunk *sfxGetCoin;       // Get coin sound
    Mix_Chunk *sfxExtraLife;     // Extra life sound
    Mix_Chunk *sfxGetFlame;      // Get flame power sound
    Mix_Chunk *sfxGetSuperBeast; // Get super beast sound
    Mix_Chunk *sfxWinLevel;      // Win level sound
    Mix_Chunk *sfxWinGame;       // Win game sound
    Mix_Chunk *sfxDie;           // Die sound
    Mix_Chunk *sfxLoseGame;      // Lose game sound
} Audio;
...
static int load_audio(void) {
    ...
    // Load background music (WAV format for better compatibility)
    // Try WAV first (most compatible)
    audio.bgMusic = Mix_LoadMUS("sound/bgmusic.wav");
    if (!audio.bgMusic) {
        fprintf(stderr, "Warning: Could not load sound/bgmusic.wav: %s\n", Mix_GetError());
        // Try MIDI as fallback (may not work on all systems)
        audio.bgMusic = Mix_LoadMUS("sound/bgmusic.mid");
        if (!audio.bgMusic) {
            fprintf(stderr, "Warning: Could not load sound/bgmusic.mid: %s\n", Mix_GetError());
        }
    }
    
    // Load sound effects (WAV files - can be converted from 8-bit samples)
    audio.sfxKillBaddie = Mix_LoadWAV("sound/sfx_kill_baddie.wav");
    audio.sfxBreakBarrel = Mix_LoadWAV("sound/sfx_break_barrel.wav");
    audio.sfxGetCoin = Mix_LoadWAV("sound/sfx_get_coin.wav");
    audio.sfxExtraLife = Mix_LoadWAV("sound/sfx_extra_life.wav");
    audio.sfxGetFlame = Mix_LoadWAV("sound/sfx_get_flame.wav");
    audio.sfxGetSuperBeast = Mix_LoadWAV("sound/sfx_get_superbeast.wav");
    audio.sfxWinLevel = Mix_LoadWAV("sound/sfx_win_level.wav");
    audio.sfxWinGame = Mix_LoadWAV("sound/sfx_win_game.wav");
    audio.sfxDie = Mix_LoadWAV("sound/sfx_die.wav");
    audio.sfxLoseGame = Mix_LoadWAV("sound/sfx_lose_game.wav");
    
    // Print warnings for missing files but don't fail
    if (!audio.sfxKillBaddie) fprintf(stderr, "Warning: sfx_kill_baddie.wav not found\n");
    if (!audio.sfxBreakBarrel) fprintf(stderr, "Warning: sfx_break_barrel.wav not found\n");
    if (!audio.sfxGetCoin) fprintf(stderr, "Warning: sfx_get_coin.wav not found\n");
    if (!audio.sfxExtraLife) fprintf(stderr, "Warning: sfx_extra_life.wav not found\n");
    if (!audio.sfxGetFlame) fprintf(stderr, "Warning: sfx_get_flame.wav not found\n");
    if (!audio.sfxGetSuperBeast) fprintf(stderr, "Warning: sfx_get_superbeast.wav not found\n");
    if (!audio.sfxWinLevel) fprintf(stderr, "Warning: sfx_win_level.wav not found\n");
    if (!audio.sfxWinGame) fprintf(stderr, "Warning: sfx_win_game.wav not found\n");
    if (!audio.sfxDie) fprintf(stderr, "Warning: sfx_die.wav not found\n");
    if (!audio.sfxLoseGame) fprintf(stderr, "Warning: sfx_lose_game.wav not found\n");
...
static void cleanup_audio(void) {
    ...
    if (audio.sfxKillBaddie) Mix_FreeChunk(audio.sfxKillBaddie);
    if (audio.sfxBreakBarrel) Mix_FreeChunk(audio.sfxBreakBarrel);
    if (audio.sfxGetCoin) Mix_FreeChunk(audio.sfxGetCoin);
    if (audio.sfxExtraLife) Mix_FreeChunk(audio.sfxExtraLife);
    if (audio.sfxGetFlame) Mix_FreeChunk(audio.sfxGetFlame);
    if (audio.sfxGetSuperBeast) Mix_FreeChunk(audio.sfxGetSuperBeast);
    if (audio.sfxWinLevel) Mix_FreeChunk(audio.sfxWinLevel);
    if (audio.sfxWinGame) Mix_FreeChunk(audio.sfxWinGame);
    if (audio.sfxDie) Mix_FreeChunk(audio.sfxDie);
    if (audio.sfxLoseGame) Mix_FreeChunk(audio.sfxLoseGame);
    Mix_CloseAudio();
}
...
static int load_sprites(SDL_Renderer *renderer, Sprites *s) {
    // Load sprites.png
    SDL_Texture *tex = IMG_LoadTexture(renderer, "sprites/vex.png");
    if (!tex) {
        fprintf(stderr, "IMG_LoadTexture(sprites/vex.png) failed: %s\n", IMG_GetError());
        return -1;
    }
    s->tex = tex;
    ...
    if (s->tileW <= 0 || s->tileH <= 0) {
        fprintf(stderr, "Invalid tile size from sprites/vex.png: %dx%d\n", s->tileW, s->tileH);
        return -1;
    }
    printf("Loaded sprites/vex.png: %dx%d, tile %dx%d\n", tw, th, s->tileW, s->tileH);
    fflush(stdout);

    // Map to your described layout:
    // Hero (from sprites/vex.png - will be replaced by hero.png when powered up)
    ...
    s->barrelBroken = sprite_tile(s, 5, 1); // broken barrel (sprites/vex.png column 5, row 1)
    ...
    // Load hero.png
    s->heroTex = IMG_LoadTexture(renderer, "sprites/hero.png");
    if (!s->heroTex) {
        fprintf(stderr, "IMG_LoadTexture(sprites/hero.png) failed: %s\n", IMG_GetError());
        return -1;
    }
    int htw = 0, hth = 0;
    if (SDL_QueryTexture(s->heroTex, NULL, NULL, &htw, &hth) != 0) {
        fprintf(stderr, "SDL_QueryTexture sprites/hero.png failed: %s\n", SDL_GetError());
        return -1;
    }
    // Assuming hero.png is 6 columns x 4 rows (normal, super beast, sword+super, flame)
    s->heroTileW = htw / 6;
    s->heroTileH = hth / 4;
    printf("Loaded sprites/hero.png: %dx%d, tile %dx%d\n", htw, hth, s->heroTileW, s->heroTileH);
    fflush(stdout);

    // Load power.png
    s->powerTex = IMG_LoadTexture(renderer, "sprites/power.png");
    if (!s->powerTex) {
        fprintf(stderr, "IMG_LoadTexture(sprites/power.png) failed: %s\n", IMG_GetError());
        return -1;
    }
    int ptw = 0, pth = 0;
    if (SDL_QueryTexture(s->powerTex, NULL, NULL, &ptw, &pth) != 0) {
        fprintf(stderr, "SDL_QueryTexture sprites/power.png failed: %s\n", SDL_GetError());
        return -1;
    }
    // Assuming power.png is 2 columns x 2 rows
    s->powerTileW = ptw / 2;
    s->powerTileH = pth / 2;
    printf("Loaded sprites/power.png: %dx%d, tile %dx%d\n", ptw, pth, s->powerTileW, s->powerTileH);
    fflush(stdout);

    // Power-up sprites
    // power.png layout: 0:0 sword, 0:1 lightning, 1:0 fire, 1:1 heart
    s->powerSword = sprite_tile_custom(s->powerTileW, s->powerTileH, 0, 0);
    s->powerFlame = sprite_tile_custom(s->powerTileW, s->powerTileH, 0, 1);
    s->powerLightning = sprite_tile_custom(s->powerTileW, s->powerTileH, 1, 0);
    s->powerHeart = sprite_tile_custom(s->powerTileW, s->powerTileH, 1, 1);

    // Load baddies.png (with transparency already baked in)
    s->baddiesTex = IMG_LoadTexture(renderer, "sprites/baddies.png");
    if (!s->baddiesTex) {
        fprintf(stderr, "IMG_LoadTexture(sprites/baddies.png) failed: %s\n", IMG_GetError());
        return -1;
    }
    // PNG already has alpha channel, but ensure blend mode is set for transparency
    SDL_SetTextureBlendMode(s->baddiesTex, SDL_BLENDMODE_BLEND);
    int btw = 0, bth = 0;
    if (SDL_QueryTexture(s->baddiesTex, NULL, NULL, &btw, &bth) != 0) {
        fprintf(stderr, "SDL_QueryTexture sprites/baddies.png failed: %s\n", SDL_GetError());
        return -1;
    }
    // Assuming baddies.png is 8 columns x 6 rows (based on description: slimes, golems, imps, knights, goblins, bats)
    s->baddieTileW = btw / 8;
    s->baddieTileH = bth / 6;
    printf("Loaded sprites/baddies.png: %dx%d, tile %dx%d\n", btw, bth, s->baddieTileW, s->baddieTileH);
    fflush(stdout);

    // Load backgrounds (single file vex-bgs.jpg with 4 backgrounds in 2x2 grid)
    s->bgTex = IMG_LoadTexture(renderer, "sprites/vex-bgs.jpg");
    if (!s->bgTex) {
        fprintf(stderr, "Warning: Could not load sprites/vex-bgs.jpg: %s\n", IMG_GetError());
        s->bgTex = NULL; // Background is optional
    } else {
        int bgw = 0, bgh = 0;
        if (SDL_QueryTexture(s->bgTex, NULL, NULL, &bgw, &bgh) != 0) {
            fprintf(stderr, "SDL_QueryTexture sprites/vex-bgs.jpg failed: %s\n", IMG_GetError());
            ...
            s->bgTileW = bgw / 2;
            s->bgTileH = bgh / 2;
            printf("Loaded sprites/vex-bgs.jpg: %dx%d, each bg %dx%d\n", bgw, bgh, s->bgTileW, s->bgTileH);
            fflush(stdout);
        }
    }
...
static void handle_player_platform_collisions(Game *g, float dt) {
    ...
    if (p->rect.y + p->rect.h > WINDOW_H) {
        ...
        if (g->lives <= 0) {
            g->gameOver = 1;
            if (audio.sfxLoseGame) Mix_PlayChannel(-1, audio.sfxLoseGame, 0);
        } else {
            g->gameOver = 1; // show death screen; restart via space
            if (audio.sfxDie) Mix_PlayChannel(-1, audio.sfxDie, 0);
        }
    }
}
    ...
    for (int i = 0; i < MAX_BARRELS; i++) {
        ...
        if (!g->gameOver && !g->win && rects_intersect(&b->rect, &g->player.rect)) {
            if (g->player.hasFlame || g->player.hasSuperBeast) {
                ...
                if (audio.sfxBreakBarrel) Mix_PlayChannel(-1, audio.sfxBreakBarrel, 0);
                // Check for extra life after score increase
                while (g->score >= g->nextLifeScore) {
                    g->lives++;
                    g->nextLifeScore += 5000;
                    if (audio.sfxExtraLife) Mix_PlayChannel(-1, audio.sfxExtraLife, 0);
                }
...
static void update_baddies(Game *g, float dt) {
    for (int i = 0; i < MAX_BADDIES; i++) {
        ...
        // Check if baddie is on a ladder
        int onLadder = 0;
        float baddieCenterX = bad->rect.x + bad->rect.w * 0.5f;
        float baddieCenterY = bad->rect.y + bad->rect.h * 0.5f;
        for (int i = 0; i < g->numLadders; i++) {
            const RectF *lad = &g->ladders[i].rect;
            if (baddieCenterX >= lad->x && baddieCenterX <= lad->x + lad->w &&
                baddieCenterY >= lad->y && baddieCenterY <= lad->y + lad->h) {
                onLadder = 1;
                break;
            }
        }
        ...
        float playerCenterY = g->player.rect.y + g->player.rect.h * 0.5f;
        float dx = playerCenterX - baddieCenterX;
        float dy = playerCenterY - baddieCenterY;
        
        // If on ladder and player is below, slide down the ladder
        if (onLadder && dy > 20.0f) {
            // Slide down ladder (faster than climbing)
            bad->rect.y += 250.0f * dt; // Slide down speed
            bad->vy = 0; // Disable gravity while sliding
            bad->onGround = 0;
            
            // Also move toward player horizontally on ladder
            if (fabsf(dx) > 20.0f) {
                if (dx > 0) {
                    bad->facing = 1;
                } else {
            ...
            // Horizontal movement (only if not on ladder sliding)
            bad->rect.x += bad->vx * dt;
        ...
        if (!g->gameOver && !g->win && rects_intersect(&bad->rect, &g->player.rect)) {
            ...
            if (killed) {
                ...
                bad->vx = 0;
                bad->vy = 0;
                g->score += 500; // Bonus for killing baddie (500 points)
                if (audio.sfxKillBaddie) Mix_PlayChannel(-1, audio.sfxKillBaddie, 0);
                
                // Spawn coins exploding from baddie death location
                float coinCenterX = bad->rect.x + bad->rect.w * 0.5f;
                float coinCenterY = bad->rect.y + bad->rect.h * 0.5f;
                int coinsToSpawn = 5 + (g->level % 3); // 5-7 coins
                for (int c = 0; c < coinsToSpawn && g->numCoins < MAX_COINS; c++) {
                    // Find an inactive coin slot
                    int coinIdx = -1;
                    for (int i = 0; i < MAX_COINS; i++) {
                        if (!g->coins[i].active) {
                            coinIdx = i;
                    ...
                    if (coinIdx < 0) break; // No more slots
                    
                    Coin *coin = &g->coins[coinIdx];
                    coin->active = 1;
                    coin->rect.w = (float)WORLD_TILE * 0.8f;
                    coin->rect.h = (float)WORLD_TILE * 0.8f;
                    coin->rect.x = coinCenterX - coin->rect.w * 0.5f;
                    coin->rect.y = coinCenterY - coin->rect.h * 0.5f;
                    
                    // Explode in a circle pattern
                    float angle = (float)(c * 2.0 * M_PI / coinsToSpawn);
                    float speed = 200.0f + 50.0f * (float)(c % 2); // Vary speed
                    coin->vx = cosf(angle) * speed;
                    coin->vy = sinf(angle) * speed - 100.0f; // Slight upward bias
                    coin->lifetime = 3.0f; // Coins disappear after 3 seconds if not collected
                    
                    if (g->numCoins < MAX_COINS) g->numCoins++;
                }
...
static void update_game(Game *g, float dt, int moveLeft, int moveRight, int jump, int climbUp, int climbDown, int attack) {
    ...
    if (!g->win && rects_intersect(&p->rect, &g->goal)) {
        ...
        g->score += 1000;
        if (g->level >= 25) {
            if (audio.sfxWinGame) Mix_PlayChannel(-1, audio.sfxWinGame, 0);
        } else {
            if (audio.sfxWinLevel) Mix_PlayChannel(-1, audio.sfxWinLevel, 0);
        }
        // Check for extra life after score increase
        while (g->score >= g->nextLifeScore) {
            g->lives++;
    ...
    // Coins collection and physics
    for (int i = 0; i < MAX_COINS; i++) {
        Coin *coin = &g->coins[i];
        if (!coin->active) continue;
        
        // Update exploding coins (those with velocity)
        if (coin->vx != 0.0f || coin->vy != 0.0f) {
            // Apply gravity to exploding coins
            coin->vy += g->gravity * dt * 0.5f; // Lighter gravity for coins
            
            // Update position
            coin->rect.x += coin->vx * dt;
            coin->rect.y += coin->vy * dt;
            
            // Friction/damping on velocity
            coin->vx *= 0.98f;
            coin->vy *= 0.98f;
            
            // Platform collisions for exploding coins
            for (int p = 0; p < g->numPlatforms; p++) {
                RectF *plat = &g->platforms[p].rect;
                if (rects_intersect(&coin->rect, plat)) {
                    if (coin->vy > 0) {
                        // Landing on platform
                        coin->rect.y = plat->y - coin->rect.h;
                        coin->vy *= -0.5f; // Bounce with reduced velocity
                        if (fabsf(coin->vy) < 50.0f) {
                            coin->vy = 0; // Stop small bounces
                        }
                    }
                }
            }
            
            // Update lifetime for exploding coins
            if (coin->lifetime > 0) {
                coin->lifetime -= dt;
                if (coin->lifetime <= 0) {
                    coin->active = 0;
                    if (g->numCoins > 0) g->numCoins--;
                }
            }
        }
        
        // Check collection
        if (coin->active && rects_intersect(&p->rect, &coin->rect)) {
            coin->active = 0;
            coin->vx = 0;
            coin->vy = 0;
            g->coinsCollected++;
            g->score += 100;
            if (audio.sfxGetCoin) Mix_PlayChannel(-1, audio.sfxGetCoin, 0);
        ...
        if (rects_intersect(&p->rect, &pu->rect)) {
            ...
            switch (pu->type) {
                ...
                case POWER_FLAME:
                    ...
                    if (audio.sfxGetFlame) Mix_PlayChannel(-1, audio.sfxGetFlame, 0);
                    break;
                case POWER_LIGHTNING:
                    g->player.hasSuperBeast = 1;
                    g->player.superBeastTimer = 30.0f; // 30 seconds
                    if (audio.sfxGetSuperBeast) Mix_PlayChannel(-1, audio.sfxGetSuperBeast, 0);
                    break;
                case POWER_HEART:
                    g->lives++;
                    if (audio.sfxExtraLife) Mix_PlayChannel(-1, audio.sfxExtraLife, 0);
...
// Helper to get platform offset for sprite alignment (similar to player)
static float get_platform_offset(const Game *g, float y, float h) {
    if (g->numPlatforms == 0) return 0.0f;
    
    // Check if on ground floor (platforms[0])
    const RectF *groundPlat = &g->platforms[0].rect;
    float bottom = y + h;
    if (fabsf(bottom - groundPlat->y) < 2.0f) {
        return 24.0f; // Ground floor offset
    }
    
    // Check if on any other platform
    for (int i = 1; i < g->numPlatforms; i++) {
        const RectF *plat = &g->platforms[i].rect;
        if (fabsf(bottom - plat->y) < 2.0f) {
            return 8.0f; // Regular platform offset
        }
    }
    
    return 0.0f; // Not on platform
}
...
static void render_game(SDL_Renderer *renderer, const Game *g, const Sprites *s) {
    ...
    {
        // Find the platform the princess is on
        float princessY = g->goal.y + g->goal.h - WORLD_TILE; // stand on platform
        float platformOffset = 8.0f; // Default to regular platform offset
        
        // Check which platform the princess is on
        if (g->numPlatforms > 0) {
            const RectF *topPlat = &g->platforms[g->numPlatforms - 1].rect;
            if (fabsf(princessY + WORLD_TILE - topPlat->y) < 2.0f) {
                // On a platform - check if it's ground floor
                if (topPlat == &g->platforms[0].rect) {
                    platformOffset = 24.0f;
                } else {
                    platformOffset = 8.0f;
                }
            }
        ...
        float gy = princessY + platformOffset; // Apply platform offset
    ...
    for (int i = 0; i < MAX_BARRELS; i++) {
        ...
        RectF barrelRenderRect = g->barrels[i].rect;
        if (!g->barrels[i].broken) {
            // Apply platform offset for alignment (only when not broken)
            float offset = get_platform_offset(g, g->barrels[i].rect.y, g->barrels[i].rect.h);
            barrelRenderRect.y += offset;
        }
        render_sprite(renderer, s->tex, barrelSrc, &barrelRenderRect, SDL_FLIP_NONE);
    }
    ...
    for (int i = 0; i < MAX_BADDIES; i++) {
        ...
        if (bad->dying) {
            ...
            // Apply platform offset for alignment (only when not dying)
            float offset = get_platform_offset(g, bad->rect.y, bad->rect.h);
            renderRect.y += offset;
        }
    ...
    SDL_Texture *heroTexture = s->tex; // Default to sprites/vex.png
    ...
    if (heroTexture == s->heroTex) {
        ...
        // Use sprites/vex.png (original sprites)
...
int main(int argc, char **argv) {
    ...
    if (load_audio() != 0) {
        ...
        // Start background music (loop it)
        if (audio.bgMusic) {
            Mix_PlayMusic(audio.bgMusic, -1); // -1 = loop indefinitely
