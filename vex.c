...
static int load_audio(void) {
    ...
    audio.bgMusic = Mix_LoadMUS("bgmusic.wav");
    if (!audio.bgMusic) {
        fprintf(stderr, "Warning: Could not load bgmusic.wav: %s\n", Mix_GetError());
        // Try MIDI as fallback (may not work on all systems)
        audio.bgMusic = Mix_LoadMUS("bgmusic.mid");
        if (!audio.bgMusic) {
            fprintf(stderr, "Warning: Could not load bgmusic.mid: %s\n", Mix_GetError());
        }
    }
    
    // Load sound effects (WAV files - can be converted from 8-bit samples)
    audio.sfxKillBaddie = Mix_LoadWAV("sfx_kill_baddie.wav");
    audio.sfxBreakBarrel = Mix_LoadWAV("sfx_break_barrel.wav");
    audio.sfxGetCoin = Mix_LoadWAV("sfx_get_coin.wav");
    audio.sfxExtraLife = Mix_LoadWAV("sfx_extra_life.wav");
    audio.sfxGetFlame = Mix_LoadWAV("sfx_get_flame.wav");
    audio.sfxGetSuperBeast = Mix_LoadWAV("sfx_get_superbeast.wav");
    audio.sfxWinLevel = Mix_LoadWAV("sfx_win_level.wav");
    audio.sfxWinGame = Mix_LoadWAV("sfx_win_game.wav");
    audio.sfxDie = Mix_LoadWAV("sfx_die.wav");
    audio.sfxLoseGame = Mix_LoadWAV("sfx_lose_game.wav");
...
static int load_sprites(SDL_Renderer *renderer, Sprites *s) {
    ...
    s->baddiesTex = IMG_LoadTexture(renderer, "baddies.png");
    if (!s->baddiesTex) {
        fprintf(stderr, "IMG_LoadTexture(baddies.png) failed: %s\n", IMG_GetError());
        return -1;
    }
